...
export const ProductListCard: React.FC<Props> = ({
  ...
  return (
    <div className={cn('relative group', className)}>
      {product.id && <div className='absolute right-2 top-2 z-30'>
        <Favorites size={18} productId={product.id} className='bg-white rounded-full p-1' />
      </div>}
      ...
      <Link href={`/product/${product.slug}`}>
        ...
        >
          {visibleImages.length > 0 && (
            <div className="relative bg-neutral-100 pb-2 overflow-hidden">
              ...
              <div className="absolute px-4 flex gap-2 left-0 bottom-1 w-full h-[2px] z-20 opacity-0 group-hover:opacity-100 transition duration-150">
                {visibleImages.map((img) => (
                  <div
                    key={img.id}
                    className={`w-full ${
                      activeImg === Number(img.id)
                        ? 'bg-[#00A3CB]'
                        : 'bg-[#cbcbcb]'
                    }`}
                  />
        ...
        <span className="text-[14px] text-[#1E1E1E] block mt-[8px] truncate font-medium">
          {product.name}
        </span>
